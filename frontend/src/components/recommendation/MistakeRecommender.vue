<template>
  <div class="mistake-recommender">
    <h4 class="section-title">
      <el-icon><WarningFilled /></el-icon> 相关错题
    </h4>
    ...
    <div v-else-if="items.length > 0" class="rec-list">
      <div v-for="item in items" :key="item.id" class="rec-card mistake" @click="onClick(item)">
        <div class="rec-card__name">{{ item.knowledgeNodeName }}</div>
        <div class="rec-card__page" v-if="item.pageTitle">{{ item.pageTitle }}</div>
        <div class="rec-card__reason">{{ item.reason }}</div>
      </div>
    </div>
    <div v-else class="empty-hint">暂无错题记录</div>
  </div>
</template>
...
watch(() => treeStore.selectedNodeId, async (nodeId) => {
  ...
  loading.value = true
  try {
    items.value = await recommendationApi.getMistakes(nodeId)
  } finally {
    loading.value = false
  }
}, { immediate: true })
